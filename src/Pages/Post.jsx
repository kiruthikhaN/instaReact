...
function Post (){
    ...
    for (let i=0; i<User.length; i++){
        post = User[i].posts.find(post=>post.id==id);
        if(post)break;
    } */  
   let curUser = User.find(tUser=> tUser.username==username);
   let post = curUser.posts.find(cPost=> cPost.id==id);
    return(
        <>
        <div className="w-[600px] mx-auto mt-12 flex gap-4">
        <img src={curUser.image} className="h-14 w-14 rounded-full object-cover m" alt="" />
       <div className="">
       <p>{curUser.name}</p>
       <p className="text-sm text-gray-600 m-0">{curUser.profession}</p>
       </div>
       </div>
